        ...
        <nav className={theme.glassNav}>
            <div className="flex justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-5">
                    ...
                    {/* THEME TOGGLE */}
                    <button onClick={toggleTheme} className={theme.iconBtn}>
                                    ...
                                    <div className={`absolute top-full right-0 mt-2 w-48 rounded-xl shadow-2xl border overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 ${isDarkMode ? 'bg-[#0a1410] border-white/10' : 'bg-white border-slate-200'}`}>
                                        <button
                                            onClick={() => {
                                                ...
                                                // setDashboardTab('profile'); // This needs to be handled. Ideally NavigateTo supports params or we pass a setter.
                                                // For now we just go to dashboard. The user can click profile there.
                                                setIsProfileDropdownOpen(false);
                                            }}
            ...
            {isMobileMenuOpen && (
                <div className={`absolute top-full left-0 w-full mt-4 border rounded-2xl overflow-hidden shadow-2xl p-4 flex flex-col space-y-2 md:hidden animate-in fade-in slide-in-from-top-5 z-50 ${isDarkMode ? 'bg-[#0a1410] border-white/10' : 'bg-white border-slate-200'}`}>
                    <button onClick={() => { navigateTo('landing'); setIsMobileMenuOpen(false); }} className={`p-3 text-left rounded-xl font-medium ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>Home</button>
                    <button onClick={() => { navigateTo('events'); setIsMobileMenuOpen(false); }} className={`p-3 text-left rounded-xl font-medium ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>Events</button>
                    {userRole !== 'guest' && (
                        <button onClick={() => { navigateTo('dashboard'); setIsMobileMenuOpen(false); }} className={`p-3 text-left rounded-xl font-medium ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>Dashboard</button>
                    )}
                    {userRole === 'guest' ? (
                        <button onClick={() => { navigateTo('login'); setIsMobileMenuOpen(false); }} className={`p-3 text-left text-emerald-500 rounded-xl font-bold ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>Log In</button>
                    ) : (
                        <button onClick={handleLogoutClick} className={`p-3 text-left text-red-500 rounded-xl font-medium ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>Log Out</button>
                    )}
                </div>
            )}
        </nav>
